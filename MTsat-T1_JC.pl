...
`minccalc -float -expr "clamp(0.5*(A[0]*$a_t1w_r/$TR_t1w-A[1]*$a_pdw_r/$TR_pdw)/(A[1]/$a_pdw_r-A[0]/$a_t1w_r),-10,100)" $t1w $pdw $r1app `;


`minccalc -float -expr "clamp(A[0]*A[1]*($TR_pdw*$a_t1w_r/$a_pdw_r-$TR_t1w*$a_pdw_r/$a_t1w_r)/(A[1]*$TR_pdw*$a_t1w_r-A[0]*$TR_t1w*$a_pdw_r),-10000,15000)" $pdw $t1w $Aapp `;
